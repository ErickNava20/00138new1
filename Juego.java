...
public class Juego<Private> extends AppCompatActivity {
    ...
    //variables del juego
    ArrayList<Integer> arrayListDesordenado;
    ImageButton primero;
    ...
    private void cargarTexto() {
        textoPuntuación = findViewById(R.id.texto_puntacion);
        puntuación = 0;
        aciertos = 0;
        textoPuntuación.setText("puntuacion: " + puntuación);
    }
    ...
    private void comprobar(int i, final ImageButton imgb) {
        if (primero == null) {
            ...
            primero.setImageResource(imagenes[arrayListDesordenado.get(1)]);
            primero.setEnabled(false);
            numeroPrimero = arrayListDesordenado.get(i);
        } else {
            bloqueo = true;
            imgb.setScaleType(ImageView.ScaleType.CENTER_CROP);
            imgb.setImageResource(imagenes[arrayListDesordenado.get(i)]);
            imgb.setEnabled(false);
            numeroSegundo = arrayListDesordenado.get(i);
    ...
    private void init()
    {
        ...
        cargarTexto();
        cargarImagenes();
        arrayListDesordenado = baraja(imagenes.length);
        for(int i=0; i< tablero.length; i++){
            tablero[i].setScaleType(ImageView.ScaleType.CENTER_CROP);
            tablero[i].setImageResource(imagenes[arrayListDesordenado.get(i)]);
        }
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
